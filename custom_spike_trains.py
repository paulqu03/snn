...
input_synapses = Synapses(poisson_inputs, E, on_pre='v_post += 4 * mV')
...
S_E_E = Synapses(E, E, on_pre='v_post += 0.5 * mV')
...
for i, j in zip(S_E_E.i, S_I_I.j):
    y_shift_exc[i] += 1
    z_shift_inh[j] += 1
...
# 2D-Konnektivitätsdiagramm mit Radio Buttons zur Auswahl der Synapsen
fig2, ax2 = plt.subplots()
plt.subplots_adjust(left=0.3)  # Platz für die Radio Buttons schaffen
ax2.set_title("2D-Synapsenverbindungen")
ax2.set_xlabel("Source Neuron Index")
ax2.set_ylabel("Target Neuron Index")

# Synapsen Scatterplots (einzeln für die Auswahl über Radio Buttons)
scatter_exc_exc = ax2.scatter(S_E_E.i, S_E_E.j, color='red', s=10, label="Excitatory to Excitatory", visible=False)
scatter_exc_inh = ax2.scatter(S_E_I.i, S_E_I.j, color='purple', s=10, label="Excitatory to Inhibitory", visible=False)
scatter_inh_inh = ax2.scatter(S_I_I.i, S_I_I.j, color='blue', s=10, label="Inhibitory to Inhibitory", visible=False)

# Radio Buttons zum Wechseln der Sichtbarkeit der Synapsentypen
rax = plt.axes([0.05, 0.4, 0.15, 0.15], frameon=True)
radio = RadioButtons(rax, ('Exc to Exc', 'Exc to Inh', 'Inh to Inh'))

# Funktion zum Umschalten der Synapsen-Ansicht
def update(label):
    scatter_exc_exc.set_visible(label == 'Exc to Exc')
    scatter_exc_inh.set_visible(label == 'Exc to Inh')
    scatter_inh_inh.set_visible(label == 'Inh to Inh')
